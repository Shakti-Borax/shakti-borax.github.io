/* ============================================================
   SHAKTI BORAX — Shared Components
   Header (fixed dark), Mobile Nav (full-screen), Footer (4-col)
   ============================================================ */

function buildHeader(activePage) {
  const links = [
    { label: "Home", href: "." },
    { label: "About Us", href: "about" },
    { label: "Products", href: "products" },
    { label: "Contact Us", href: "contact" },
  ];

  const navLinks = links
    .map(
      (l) =>
        `<a href="${l.href}" class="${l.label === activePage ? "active" : ""}">${l.label}</a>`,
    )
    .join("");

  const mobileLinks = links
    .map(
      (l) =>
        `<a href="${l.href}" class="${l.label === activePage ? "active" : ""}">${l.label}</a>`,
    )
    .join("");

  document.getElementById("header-root").innerHTML = `
    <header class="site-header">
      <div class="header-inner container">
        <a class="header-brand anim-slide" href=".">
          <img src="./images/logo.webp" alt="Shakti Borax Logo" width="48" height="48">
          <div class="brand-text">
            <span class="brand-name">SHAKTI BORAX PRIVATE LIMITED</span>

          </div>
        </a>
        <nav class="header-nav">
          ${navLinks}
          <a class="nav-iso" href="./documents/ISO-9001.pdf" target="_blank" rel="noopener noreferrer" title="ISO 9001 Certificate">
            <img src="./images/cert.webp" alt="ISO 9001 Certified" width="50" height="52">
          </a>
        </nav>
        <button class="hamburger" id="hamburgerBtn" aria-label="Open menu">
          <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"/>
          </svg>
        </button>
      </div>
    </header>

    <!-- Mobile Nav -->
    <div class="mobile-nav" id="mobileNav">
      <div class="mobile-nav-top">
        <a class="header-brand" href=".">
          <img src="./images/logo.webp" alt="Shakti Borax Logo" width="48" height="48">
          <div class="brand-text">
            <span class="brand-name">SHAKTI BORAX PRIVATE LIMITED</span>
          </div>
        </a>
        <button class="mobile-close" id="mobileCloseBtn" aria-label="Close menu">
          <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
          </svg>
        </button>
      </div>
      <div class="mobile-nav-body">
        <div class="mobile-nav-links">
          ${mobileLinks}
        </div>
        <div class="mobile-nav-iso">
          <a href="./documents/ISO-9001.pdf" target="_blank" rel="noopener noreferrer">
            <img src="./images/cert.webp" alt="ISO 9001 Certified" width="30" height="32">
            <span>ISO 9001:2015 Certification</span>
          </a>
        </div>
      </div>
    </div>
  `;

  // Wire up hamburger / close
  const hamburger = document.getElementById("hamburgerBtn");
  const mobileNav = document.getElementById("mobileNav");
  const closeBtn = document.getElementById("mobileCloseBtn");

  function openMenu() {
    mobileNav.classList.add("open");
    document.body.style.overflow = "hidden";
  }
  function closeMenu() {
    mobileNav.classList.remove("open");
    document.body.style.overflow = "";
  }

  hamburger.addEventListener("click", openMenu);
  closeBtn.addEventListener("click", closeMenu);

  // Close when clicking a nav link
  mobileNav.querySelectorAll(".mobile-nav-links a").forEach((a) => {
    a.addEventListener("click", closeMenu);
  });
}

function buildFooter(activePage) {
  const links = [
    { label: "Home", href: "." },
    { label: "About Us", href: "about" },
    { label: "Products", href: "products" },
    { label: "Contact Us", href: "contact" },
  ];

  const menuLinks = links
    .map(
      (l) =>
        `<a href="${l.href}" class="${l.label === activePage ? "active" : ""}">${l.label}</a>`,
    )
    .join("");

  document.getElementById("footer-root").innerHTML = `
    <footer class="site-footer">
      <div class="container">
        <div class="footer-grid">

          <!-- Brand -->
          <div>
            <div class="footer-brand-logo">
              <img src="./images/logo.webp" alt="Shakti Borax Logo" width="48" height="48">
              <span class="footer-brand-name">SHAKTI BORAX PRIVATE LIMITED</span>
            </div>
            <p class="footer-brand-desc">
              Established in 1990, Shakti Borax Private Limited is the leading manufacturer and importer of Boron products in North India. An ISO 9001:2015 certified company, supplying industries since 1994.
            </p>
          </div>

          <!-- Menu -->
          <div class="md-ml-4">
            <h3 class="footer-col-title">Menu</h3>
            <div class="footer-menu-grid">${menuLinks}</div>
          </div>

          <!-- Office -->
          <div>
            <h3 class="footer-col-title">Contact</h3>
            <div class="footer-office-item">
              <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
              <span>Delhi, India</span>
            </div>
            <div class="footer-office-item">
              <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
              <a href="mailto:info@shaktiborax.in">info@shaktiborax.in</a>
            </div>
          </div>

          <!-- Social -->
          <div>
            <h3 class="footer-col-title">Connect With Us</h3>
            <div class="social-row">
              <a class="social-btn fb" href="https://www.facebook.com/shaktiborax.in/" target="_blank" rel="noopener noreferrer" title="Facebook">
                <svg fill="currentColor" viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>
              </a>
              <a class="social-btn li" href="https://www.linkedin.com/company/shaktiborax" target="_blank" rel="noopener noreferrer" title="LinkedIn">
                <svg fill="currentColor" viewBox="0 0 24 24"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z"/><circle cx="4" cy="4" r="2"/></svg>
              </a>
              <a class="social-btn wa" href="https://wa.me/919810017215" target="_blank" rel="noopener noreferrer" title="WhatsApp">
                <svg fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
              </a>
            </div>
          </div>

        </div>
        <div class="footer-bottom">
          <p>© ${new Date().getFullYear()} Shakti Borax Private Limited. All rights reserved.</p>
        </div>
      </div>
    </footer>
  `;

  // Both header and footer are now in the DOM — safe to init scroll animations
  initAnimations();
}

function openDoc(url) {
  window.open(
    url,
    "_blank",
    "width=950,height=720,toolbar=no,scrollbars=yes,resizable=yes",
  );
}

// Called at the end of buildFooter so all HTML is already injected.
// Elements already on screen get .in-view synchronously (no blank flash).
// Below-fold elements are watched by IntersectionObserver.
function initAnimations() {
  function trigger(el) {
    el.classList.add("in-view");
    el.addEventListener(
      "animationend",
      function () {
        el.style.willChange = "auto";
      },
      { once: true },
    );
  }

  var observer = new IntersectionObserver(
    function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          trigger(entry.target);
          observer.unobserve(entry.target);
        }
      });
    },
    { threshold: 0.1, rootMargin: "0px 0px -32px 0px" },
  );

  document
    .querySelectorAll(".anim-up, .anim-scale, .anim-slide")
    .forEach(function (el) {
      var rect = el.getBoundingClientRect();
      if (rect.top < window.innerHeight && rect.bottom > 0) {
        // Already in the viewport — trigger animation immediately
        trigger(el);
      } else {
        observer.observe(el);
      }
    });
}
